...
grand_parent: Goorm
has_children: false
permalink: /docs/Bootcamp/Goorm/Customer Analytics/Customer Analytics 7/
---
...
## 개인 과제
1. 지금까지 학습한 고객 분석 기법으로 도출한 인사이트들을 다시 살펴보고, 어떤 것이 유의미하고 어떤 것이 그렇지 않은지 구분해 보세요. 각 판단에 대한 근거를 함께 정리합니다.
2. 유의미하지 않다고 판단한 인사이트를 더 가치 있게 만들려면 어떤 추가 작업이 필요한지 생각해 보고, 구체적인 개선 방안을 수립합니다.
3. 하나의 프로젝트를 선택하여, 분석 결과를 바탕으로 제품 개선의 우선순위를 정해 보세요. 이해관계자들이 "왜 이 순서인가?"를 납득할 수 있도록 판단 근거를 명확히 정리합니다.
