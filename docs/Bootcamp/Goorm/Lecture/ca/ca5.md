...
## 개인(그룹) 과제 - 택 1(둘 다 함)
1. 대중적이지 않지만 자주 접속하게 되는 제품을 선택하여 직접 사용해봅니다. 이 제품이 사용자를 반복적으로 사용하게 만드는 요소를 Hook 모델로 분석하고, 각 단계에서 어떤 경험이 중독성을 만드는지 설명해 봅니다.

2. 유용해 보이지만 사람들이 반복 사용하지 않는 제품을 선택하여 직접 사용해 봅니다. 현재 제품에서 Hook 모델의 어느 단계가 약한지 파악하고, 사용자가 습관적으로 사용할 수 있도록 각 단계를 개선하는 방안을 제시해 봅니다.
